...
문제를 더 열심히 푸는 대신, 문제가 놓인 좌표계를 바꿔버린다는 발상 때문이다. $\mathbb{Z}$ 안에서 막힌 문제가 $\mathbb{Z}[i]$로 올리면 풀린다. 보이지 않던 인수가 나타나고, 불가능하던 방정식이 열린다. 더 넓은 세계에서 보면 답은 원래 거기 있었다.

그리고 이 확장에는 조건이 있다. $\mathbb{Z}[\sqrt{-6}]$가 보여주듯, 잘못된 확장은 구조를 보존하지 못한다. 유일 인수분해라는 산술의 근간이 무너진다. 아무 파괴나 창조가 아니다 — 구조를 보존하는 확장만이 새로운 세계를 연다.
