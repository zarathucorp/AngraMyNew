...
외부 전자선이 스피너 $u, \bar{u}$이고, 꼭짓점이 결합상수 $-ie\gamma^\mu$이고, 내부 광자선이 전파인자 $\frac{-ig_{\mu\nu}}{q^2}$다. 선을 읽으면 식이 나오고, 점을 읽으면 상수가 나온다. 그것이 전부다.

## 보이게 만들자 본질이 드러났다

파인만 다이어그램은 QED를 넘어서 약력, 강력까지 확장되었고 표준모형 전체의 공용어가 되었다. 입자물리학자의 칠판에서 수식이 사라지고 그림이 남았다. 경로적분은 양자장론의 기초가 되었고, 통계역학과 양자역학을 연결했고, 양자중력 이론의 출발점이 되었다.
